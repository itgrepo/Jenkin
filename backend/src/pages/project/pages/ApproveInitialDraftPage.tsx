...
import { downloadFileServer } from "@services/globalService";

dayjs.locale("th");

export default function ApproveInitialDraftPage() {
  ...
  const loadProjects = async () => {
    try {
      ...
      // Filter projects ที่รออนุมัติร่างขั้นต้น (stage code 48.20)
      const params: any = {
        stageCode: "48.20",
        page: 1,
        limit: 100,
  ...
  const confirmAction = async () => {
    ...
    const confirmResult = await showConfirm(
      ...
      `คุณต้องการ${actionText}ร่างขั้นต้นของโครงการ "${selectedProject.nameThai || selectedProject.name}" ใช่หรือไม่?`
    );
    ...
    try {
      ...
      if (actionType === "approve") {
        ...
        const logsToCreate: Array<{
        ...
        }> = [
          {
            stageCode: "48.60",
            stageDescription:
              stageCodeList?.find((stage) => stage.code === "48.60")?.name || "",
            stageDate: approvalDate,
          },
          {
            stageCode: "48.99",
            stageDescription:
              stageCodeList?.find((stage) => stage.code === "48.99")?.name || "",
            stageDate: approvalDate,
          },
          {
            stageCode: "50.00",
        ...
        // อัปเดต stage code เป็น 50.00
        const newStageCode = "50.00";
        const newStageUiMsg =
        ...
        await updateProjectStage({
          ...
          approve_initial_draft_by: currentUser?.id || 0,
          approve_initial_draft_action: actionType,
          approve_initial_draft_remarks: remarks,
        });
      ...
      showSuccess("สำเร็จ", `ได้${actionText}ร่างขั้นต้นเรียบร้อยแล้ว`);
  ...
  const handleDownloadFile = async (filePath: string, fileName: string) => {
    try {
      await downloadFileServer(filePath, fileName);
    } catch (err: any) {
      console.error("Error downloading file:", err);
      showError("เกิดข้อผิดพลาด", "ไม่สามารถดาวน์โหลดไฟล์ได้");
    }
  };
  ...
  // Filter projects ที่รออนุมัติ
  const pendingProjects = projects.filter((p) => p.stageCode === "48.20");

  // Calculate stats
  // const approvedCount = projects.filter(
  //   (p) => p.stageCode === "48.60" || p.stageCode === "48.99" || p.stageCode === "50.00"
  // ).length;
  // const disapprovedCount = projects.filter(
  //   (p) => p.stageCode === "48.98"
  // ).length;
          ...
          {pendingProjects.length === 0 ? (
            <Paper sx={{ p: 4, textAlign: "center" }}>
              <Typography variant="body1" color="text.secondary">
                ไม่มีรายการร่างขั้นต้นที่รออนุมัติ
              </Typography>
            </Paper>
          ) : (
                ...
                >
                  <CardContent sx={{ p: 3 }}>
                    ...
                    <Box sx={{ mb: 2 }}>
                        ...
                        <Grid size={{ xs: 12, md: 6 }}>
                          ...
                          {project.initial_draft_date && (
                            ...
                            >
                              ...
                              <Typography variant="body2">
                                <strong>วันที่บันทึกร่างขั้นต้น:</strong>{" "}
                                {dayjs(project.initial_draft_date).format("DD/MM/YYYY")}
                              </Typography>
                            </Box>
                          )}
                        </Grid>
                      ...
                      {/* Attached Documents */}
                      {(project.initial_draft_file_path ||
                        project.initial_draft_meeting_report_file_path ||
                        project.initial_draft_questionnaire_summary_file_path ||
                        project.initial_draft_powerpoint_file_path ||
                        project.initial_draft_document_file_path) && (
                        <Box sx={{ mt: 2 }}>
        ...
        >
          ...
          <Box sx={{ p: 3 }}>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              {selectedProject?.initial_draft_date &&
                `วันที่บันทึกร่างขั้นต้น: ${dayjs(selectedProject.initial_draft_date).format("DD/MM/YYYY")}`}
            </Typography>
            <TextField
              fullWidth
              multiline
