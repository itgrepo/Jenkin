...
interface CancelStandardResolutionDialogProps {
  open: boolean;
  onClose: () => void;
  review: Project | null;
  onSuccess?: () => void;
...
const CancelStandardResolutionDialog: React.FC<CancelStandardResolutionDialogProps> = ({
  open,
  onClose,
  review,
  onSuccess,
  ...
  const handleSave = async () => {
    ...
    const confirm = await showConfirm(
      "ยืนยันการบันทึก",
      "คุณต้องการบันทึกข้อมูลลงมติยกเลิกมาตรฐานหรือไม่?"
    );
    ...
    try {
      ...
      // บันทึก log 95.20, 95.60
      logsToCreate.push({
        projectId: review.id,
        stageCode: "95.20",
        stageDescription: stageCodeList?.find((stage) => stage.code === "95.20")?.name || "",
      ...
      logsToCreate.push({
        projectId: review.id,
        stageCode: "95.60",
        stageDescription: stageCodeList?.find((stage) => stage.code === "95.60")?.name || "",
        stageDate: summaryDateStr,
        remarks: remarks || undefined,
      });

      if (decision === "confirm") {
        // ยืนยันใช้มาตรฐานเดิม
        logsToCreate.push({
          projectId: review.id,
          stageCode: "95.92",
          stageDescription: stageCodeList?.find((stage) => stage.code === "95.92")?.name || "",
          stageDate: summaryDateStr,
          remarks: remarks || undefined,
        });
        newStageCode = "95.92";
        ...
        logsToCreate.push({
          projectId: review.id,
          stageCode: "95.99",
          stageDescription: stageCodeList?.find((stage) => stage.code === "95.99")?.name || "",
          stageDate: summaryDateStr,
          remarks: remarks || undefined,
        });
        newStageCode = "95.99";
        ...
        // TODO: บันทึกข้อมูลใน Table tb3_tis, tb3_tis_status, tb3_keywold, tb3_tis_cancel, tb3_log
      }
      ...
      await updateProjectReview({
        ...
        review_cancel_summary_remark: remarks,
      });
      ...
      showSuccess("บันทึกสำเร็จ", "บันทึกข้อมูลลงมติยกเลิกมาตรฐานเรียบร้อยแล้ว");
      ...
      console.error("Error saving cancel resolution:", err);
      showError(
        "เกิดข้อผิดพลาด",
...
export default CancelStandardResolutionDialog;
